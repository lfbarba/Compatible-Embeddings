...
it connected in such a way that these vertices and edges can be added to $G_1,\ldots,G_k$ to obtain planar drawings isomorphic to the augmentation of $\mathcal G$.  We show that
...
to animate the change of expression.  
...
be connected. In many applications of morphing the input graph is not connected. Before these
...
This motivates the elegant theoretical question studied in the current paper.
...
With these conventions, we are ready to state the main problem studied in this paper.  Given $k$
...
$k$ planar drawings of $\mathcal{G}$), what is the size of the smallest compatible augmentation of $\mathcal G$?  
To answer this question, we show that, if $\mathcal{G}$ has
...
there is work on isomorphic triangulations of polygons.   In this setting, the graph
...
To guide the reader, we give a rough sketch of our upper bound proof: For each component,
...
Due to space constraints, some of the proofs are deferred to the Appendix.
...
Given an arbitrary order $(v_1, v_2, \ldots, v_n)$ of the points of $S$, we want to construct a path $R$ that connects them in this order  such that 
$$|R|  = O\left(\sum_{i=1}^{n-1} |\rank(v_i) - \rank(v_{i+1})| \right)\ .$$ This paper (over-)uses the $|\cdot|$ operator in several different ways, depending on its argument.  For a real number, $x$, $|x|$ is the absolute value of $x$.  For a walk, $R=(r_0,\ldots,r_k)$, $|R|=k$ is the number of edges traversed by $R$. For a (weakly-)simple polygon, $P$, whose vertices---as encountered in a counterclockwise traversal---are $(a_1,\ldots,a_k)$, $|P|=k$ denotes the number of edges of~$P$.
...
Let $\pi_j$ be the closure of the set obtained from $\pi$ by removing the cone $\Delta_h$, for every $h\in\{j,j+1,\ldots,n\}$; see Figure~\ref{fig:Dented Halfspace} (right). That is, $\pi_j$ is a halfspace with dents made by the removal of $n-j+1$ cones.
...
For each $1\leq i\leq n$, we can construct a path $R_i$ of length $O(n^{2-1/k})$ that connects every point of $G_i$ so that $G_i\cup R_i$ is plane. Moreover, for each $1\leq i < j \leq n$, $G_i\cup R_i$ and $G_j\cup R_j$ are~isomorphic.
\end{theorem}
\vspace{-.2in}
\begin{proof}
...
For each $G_i$, we use Lemma~\ref{lemma:Compatible augmentation
...
Let $\Delta_i$ be the convex hull of $\ch{N_i\cup \{a_i\}}$, i.e., $\Delta_i$ is a ``cone'' with apex at $a_i$; see Figure~\ref{fig:Neighborhood} (middle, right). We deliberately misuse the word ``cone'' here because the ``cones'' $\Delta_1,\ldots,\Delta_r$ in this section play the same roles as the cones $\Delta_1,\ldots,\Delta_n$ in Section~\ref{section:Trivial components}.
...
Finally, for each $1\leq i\leq n$, we reduce the size of the neighborhood $N_i$ by reducing $\tau$ by a factor of two and updating the cone $\Delta_i$ accordingly, i.e., we narrow the cone $\Delta_i$; see Figure~\ref{fig:Skip Component} (right). 
...
For now, we will assume that, in these drawings, every component of $\mathcal G$ has at least one vertex incident on the outer face.  In the Appendix, we show how to remove this restriction.
...
of $\mathcal G$.  Because the drawings $G_1,\ldots,G_k$ of $\mathcal G$ that we consider are isomorphic, we can select
...
For each $j\in\{1,\ldots,r\}$, let $x_j\in \mathbb{R}^k$ be a point that corresponds to the component $C_j$ such that 
...
Note that the order of the points in $P$ induces an order on the components of $\mathcal G$ and hence, an order on the attachment corners of each $G_i$. 
...
Because $\sigma_{G_i}(a_j, a_{j+1}) = |\sigma_{G_i}(a_1, a_{j+1}) - \sigma_{G_i}(a_1, a_j)|$ by Lemma~\ref{lemma:Contained in integer grid} and since $\rank_i(j) = \sigma_{G_i}(a_1, a_j)$, we get that  $\sigma_{G_i}(a_j, a_{j+1}) = |\rank_i(j+1) - \rank_i(j)|$.
...
Thus, the path $R_i$ has length $O\left(\sum_{j=1}^{n-1} |\rank_i(j+1) - \rank_i(j)|\right) = O(\sum_{j=1}^{n-1} d_j)$. 
Because $\sum_{j=1}^{n-1} d_j = |P| = O(nr^{1-1/k})$, the length $R_i$ is $O(nr^{1-1/k})$.

Because each $R_i$ visits each component only at its attachment corner and from the fact that $R_i$ leaves every component to the right when oriented from $a_1$ to $a_r$, we conclude that $G_i\cup R_i$ is isomorphic to $G_j\cup R_j$ for each $1\leq i<j\leq k$.
\end{proof}
