...
problem}: We are given a labeled planar graph,
$\mathcal{G}$, with $n$ vertices that has $r>1$ connected components, and $k>1$ isomorphic planar straight-line drawings, $G_1,\ldots,G_k$, of
...
of a labeled graph $\mathcal{G}=(V,E)$ is a one-to-one function $\varphi\colon
V\to\R^2$.  A drawing is \emph{planar} if, for every pair of edges $uw$
and $xy$ in $E$, the line segment with endpoints $\varphi(u)$
and $\varphi(w)$ is either disjoint from the open line segment with endpoints
$\varphi(x)$ and $\varphi(y)$, or they share at most one endpoint.  Two planar drawings, $\varphi_1$ and
...
Instead, we will talk about a labeled graph $\mathcal{G}$ and $k$ isomorphic
...
in $G_i$ where $v$ is actually a (labeled) vertex of $\mathcal{G}$; this should
...
\begin{theorem}\label{thm:lower-bound}
  ...
  there exists a graph $\mathcal G$ having $n$ vertices, $1\leq r\leq n$ connected
  components, and $k>1$ isomorphic drawings $G_1,\ldots,G_k$ such that
