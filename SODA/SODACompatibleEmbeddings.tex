...
of edges and vertices added. As Figure~\ref{fig:bad-example} illusrates, it is not always possible to just add edges to $\mathcal G$; sometimes additional vertices are necessary.
...
We show that, if $\mathcal{G}$ has $n$ vertices and $r$ connected components, there always exists a compatible augmentation of size $O(k^{1/2}nr^{1-1/k})$. Furthermore, this bound is tight for constant values of $k$, as there exists a graph $\mathcal G$ with $r$ components and $k$
...
result on the Euclidean Travelling Salesman Problem \cite{few:shortest} implies that it has a spanning path of
...
The main open problem left by this work is that of closing the gap between our upper and lower bounds, which differ by a factor of $k^{1/2}$.
