...
\title{\MakeUppercase{Compatible Connectivity-Augmentation \newline of Planar Disconnected Graphs}\thanks{This research was partly supported by NSERC and the Ontario Ministry of Research and Innovation.}}
...
\author{Greg Aloupis,\thanks{Department of Computer Science, Tufts University, 
       ...
       Fabrizio Frati,\thanks{Department of Engineering, 
       				Roma Tre University, 
...
\maketitle
\begin{abstract}
% Motivated by applications to graph morphing, 
...
%Imagine an animator who wishes to animate a
...
%A \emph{morph} of a planar graph drawinng $G$ is a continuous deformation
%of the drawing that preserves planarity.
The 
...
%(for example in Figure~\ref{fig:faces}) 
...
a distinguished \emph{corner}, $a_i$, of this component in the drawing $G_1$. (Roughly speaking, 
a corner is a vertex of the weakly-simple polygon obtained by walking along the boundary of $\mathcal C_i$ in the drawing $G_1$.) 
This corner $a_i$ is called the \emph{attachment corner}
for component $\mathcal C_i$.  Notice that, since $G_1,\ldots,G_k$
are isomorphic, $a_i$ appears as a corner of $\mathcal C_i$ in each of $G_1,\ldots,G_k$.
...
The cones and their apexes will be redefined during each iteration of the algorithm.
...
%Each element $v_i$ in this sequence is called a \emph{corner} of $C$.
We consider the boundary of $C$, denoted by $\partial C$,\footnote{More formally, $\partial C$ is the boundary of the unbounded component of $\mathbb{R}^2\setminus C$, when we treat $C$ as the union of all its edges (line segments) and vertices (points).} to be the boundary of the weakly-simple polygon $(v_0, \ldots, v_k, v_0)$ whose
vertex set is the set of \emph{corners} of $\partial C$ (or simply of $C$).
Thus, each corner of $C$ coincides with a vertex of $\partial C$ (several corners may coincide with the same vertex of $\partial C$).
...
Note that $\partial_\varepsilon C$ is simple, provided that $\varepsilon$ is sufficiently small. In this paper, we consider only simple $\varepsilon$-fattenings; see Figure~\ref{fig:Blowing}. Note that the number of edges between two corners of $C$ along the boundary of $C$ is the same as the number of edges between the $\varepsilon$-copies of the two corners along~$\partial_\varepsilon C$.
...
For each $i\in \{1,\dots,r\}$, let $a_i$ be an arbitrary corner of $C_i$ (note that $a_i$ is adjacent to the outer face).
We call $a_i$ the \emph{attachment corner} of $C_i$.

Let $\varphi$ be the path on the corners of $G^*$ obtained by splitting $\partial G^*$ at the corner $a_1$. That is, $\varphi$ is a path that visits every corner of $G^*$ exactly once except for $a_1$, which is visited twice.
...
Given a geometric path $R = (\rho_1, \rho_2, \ldots, \rho_t)$, we say that $R$ \emph{visits a corner} $a_i$ of $G^*$ if the point $a_i$ is a vertex of $R$, i.e., if $a_i = \rho_j$ for some $1\leq j\leq t$.
If $R$ visits each attachment corner of $G^*$ exactly once, then we say that $C_i$ \emph{lies to the right of} $R$ if (1) $a_i$ is the only vertex of $C_i$ that is also a vertex of $R$, and (2) if $a_i = \rho_j$ for some $j\in\{1,\ldots,t\}$, then $\rho_{j-1}$ and $\rho_{j+1}$ appear as consecutive vertices when sorting---in the connected graph $G\cup R$---the neighbors of $a_i$ in clockwise order around $a_i$ (see Figure~\ref{figure:right-of}).
...
We now show how to construct a path $R$ that visits the attachment corners of $G^*$ in the given order, i.e., if $i < j$, then $a_i$ is visited before $a_j$ by $R$. We want to construct $R$ so that each component $C_i$ of $G$ lies to the right of~$R$ and so that $G\cup R$ is a plane geometric graph.
...
While constructing $R$, we also maintain the \emph{escape invariant} which is defined as follows. Assume that $R$ so far visits $a_1,\ldots,a_i$, for some $i\in \{1,\dots,r-1\}$. Then: 
\begin{enumerate}
\item $R$ intersects neither $\partial_\varepsilon G^*$ nor its unbounded face; 
\item for each $j\in\{i+1,\ldots,r\}$, $R$ intersects neither the simple polygon bounded by $\partial_\delta C_j$ nor the cone $\Delta_j$; \item $\Delta_h\cap \Delta_j = \emptyset$, for any distinct $h,j\in\{1,\ldots,r\}$; and 
\item $w_i$ is $R$-visible from $a_i$.
\end{enumerate}
...
Indeed, because the value of $\varepsilon$ increases while the value of $\delta$ decreases in each round, the edges of $\gamma_j$ that lie on the boundaries of some $\partial_\delta C_i$ or on $\partial_\varepsilon G^*$ cannot cross $R$ by the escape invariant.
...
%We then repeat this step on each face.
...
\begin{enumerate}
  ...
  \item Construct the point set $X$ and find the path
...
Without loss of generality, consider some edge-minimal compatible
...
interior.
For each of the $r/2$ indices $i\in\{1,\ldots,r\}$ that satisfy
...
it is a tree; $\mathcal G$ contains no cycles, so any cycle in $\mathcal H$ contains an edge not in $\mathcal G$ that could be removed.  Now, observe that if we traverse the outer face of (any plane drawing of) $\mathcal H$ then we obtain a non-simple path, $P$, that traverses each edge of $\mathcal{H}$ exactly twice. If we consider the set of maximal subpaths of $P$ with no vertex of $\mathcal G$ in their interior, we obtain a set of $r$ edge-disjoint paths, $Q_1,\ldots,Q_{r}$ and, for every component $\mathcal C_i$ of $\mathcal G$, there is a vertex of $\mathcal C_i$ that is an endpoint of at least one such path.  Therefore, from the preceding discussion, the total length of $Q_1,\ldots,Q_{r}$ is $\Omega(nr^{1-1/k})$.  But since each edge of $\mathcal H$ appears at most twice in these subpaths, we conclude that $\mathcal H$ has $\Omega(nr^{1-1/k})$ edges.  Since $\mathcal H$ is a tree, it has $\Omega(nr^{1-1/k})$ vertices.
\end{proof}

The lower bound in Theorem~\ref{thm:lower-bound} can be extended to work with values of $r$ up to $(1-\varepsilon) n$, for any constant $0<\varepsilon < 1$. 
However, our techniques do not apply for the case presented in Section~\ref{section:Trivial components} where $\mathcal G$ has no edges. It is therefore an open problem to determine if the upper bound presented in Section~\ref{section:Trivial components} is tight.
...
\cite{moran:on} which in turn uses ideas that go back to Fejes~\cite{fejes1940geometrischen} and Verblunsky~\cite{verblunsky1951shortest}. An argument of Few \cite{few:shortest}, which begins
