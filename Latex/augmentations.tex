...
\newcommand{\qed}{\ensuremath{\hfill\quad\square}}
...
\title{\Large Compatible Connectivity-Augmentation of Planar Disconnected Graphs}

\author{
Greg Aloupis \thanks{Department of Computer Science, Tufts University, 
                             \email{aloupis.greg@gmail.com}} \and
       Luis Barba \thanks{School of Computer Science, Carleton and
       D\'epartement d'Informatique, 
                          Universit\'e Libre de Bruxelles,
                          \email{lbarbafl@ulb.ac.be}}  \and
       Paz Carmi \thanks{Department of Computer Science,
                         Ben-Gurion University of the Negev,
                         \email{carmip@cs.bgu.ac.il}}\and
       Vida Dujmovi\'c \thanks{School of Computer Science 
                             and Electrical Engineering,
                             University of Ottawa,
                             \email{vida.dujmovic@uottawa.ca}} \and
       Fabrizio Frati \thanks{School of Information Technologies,
                              The University of Sydney,
                              \email{fabrizio.frati@sydney.edu.au}} \and
...
\begin{figure*}
  ...
  \caption{\small Two drawings, $G_1$ and $G_2$, of the same graph, $\mathcal{G}$, where making $\mathcal{G}$ connected requires the addition both of edges and vertices. In this case, $\mathcal G$ is made connected by adding the hollow vertex and two dashed edges.}
...
\begin{figure*}
  ...
  \end{tabular}}
  \caption{\small Computer-assisted animation frequently involves morphing between
...
Some motivation for this work comes from the
...
to happy (see Figure~\ref{fig:faces}). 
In this setting, animating the face becomes a problem of
\emph{morphing} (i.e., continuously deforming) one drawing of
a planar graph into another isomorphic drawing\footnote{In many cases, the correspondence is a byproduct
of the creation process. For example, in Figure~\ref{fig:faces}, the
second two faces were obtained by copying and then editing the first one.} of the same planar graph
while maintaining planarity of the drawing throughout the
deformation. This morphing problem has been studied extensively~\cite{alamdari.angelini.ea:morphing,angelini.dalozzo.ea:morphing,cairns:deformations,grunbaum.shephard:geometry,thomassen:deformations}.

Most morphing algorithms require that the input graph,
$\mathcal{G}$, be connected. In many applications of morphing (for example
in Figure~\ref{fig:faces}) the input graph is not connected. To apply these algorithms
directly in disconnected graphs, $\mathcal{G}$ must be augmented into a
connected graph, $\mathcal H$, but this augmentation must be compatible
with all drawings of $\mathcal{G}$.  At the same time, the complexity
...
every pair of edges $uw$ and $xy$ in $E$, the open straight-line segment with
endpoints $\psi(u)$ and $\psi(w)$ is disjoint from the open straight-line
segment with endpoints $\psi(x)$ and $\psi(y)$ and (b)~for every edge
$uw$ and every vertex $y$, $\psi(y)$ is not contained in the open straight-line
...
\begin{figure}
  ...
  \caption{\small Compatible triangulations of two 4-gons.}
...
a distinguished \emph{corner}, $a_i$, of $G_1$. 
A corner is a copy of a vertex of the
outer face identified with the
space between two consecutive edges incident to this vertex. 
The corner $a_i$ is called the \emph{attachment corner}
...
\begin{figure*}
  \begin{center}
   ...
   \end{tabular}
  \end{center}
  \caption{\small The algorithm for making a compatible augmentation of $\mathcal{G}$ works by defining corners $a_1,\ldots,a_r$, taking a spanning path on each augmented drawing $G_i^*$ that visits all corners, and using this information to compute a permutation of $a_1,\ldots,a_r$ that can be drawn efficiently in each $G_i$.}
...
First, we translate $\Delta_{j+1}$ down until its apex $u_{j+1}$ coincides with $v_{j+1}$. Let $\pi_j$ be the closure of the set obtained from $\pi$ by removing $\Delta_h$, for every $h\in\{j,j+1,\ldots,n\}$; see Figure~\ref{fig:Dented Halfspace} (middle, right). That is, $\pi_j$ is a halfspace with dents made by the removal of $n-j+1$ cones.
...
\begin{array}{l}
   \|x_v-x_u\|_\infty = \\
\quad\quad\  \max\{|\rank_{G_i}(v)-\rank_{G_i}(u)|:i\in\{1,\ldots,k\}\} \enspace .
 \end{array}
\]
...
For each $i\in \{1,\dots,n\}$, we can construct a path $R_i$ with $O(n^{2-1/k})$ vertices that connects every point of $G_i$ so that $G_i\cup R_i$ is planar. Moreover, for any distinct $i,j\in \{1,\dots,n\}$, the drawings of $G_i\cup R_i$ and $G_j\cup R_j$ are~isomorphic.
\end{theorem}
\begin{proof}
...
For each drawing $G_i$, we use Lemma~\ref{lemma:Compatible augmentation for trivial components} to draw $\mathcal{H}$ as a planar path, $R_i$,
that connects the vertices $v_1,\ldots,v_n$ in this order in $G_i$.
...
%[width=1\textwidth]
\caption{\small The graph $G$ (left); a connected augmentation, $G^*$, of $G$ (middle); and the $\varepsilon$-fattening, $\partial_\varepsilon G^*$ (right).}
\label{fig:Blowing}
\end{figure*}
...
Let $G$ be a geometric planar graph with $r$ connected components such that each component is adjacent to the outer face. While this is a particular case, it is the stepping stone to solve the general case as shown in Section~\ref{sec:Handling interior components}.

Two vertices are {\em visible} if the open straight-line segment joining them does not intersect $G$.
Let $T_G$ be a smallest set of non-crossing edges of the visibility graph of $G$ that need to be added to $G$ to make it connected.
...
Let $\varphi$ be the path on the corners of $\partial G^*$ (hence~$\varphi$ is also a walk on the vertices of $\partial G^*$) obtained by splitting $\partial G^*$ at the corner $a_1$. To avoid visiting $a_1$ twice, orient~$\varphi$ counterclockwise and prune the copy of $a_1$ that lies at the end of this path. That is, $\varphi$ becomes a path that visits every corner of $\partial G^*$ exactly once.
...
\]
which we call the \emph{cost} of going from $u$ to $v$.  The definition of $\sigma_G(u,v)$ is designed to capture the fact that, if $u$ and $v$ occur consecutively on the path $R$ we construct, then the portion of $R$ between $u$ and $v$ will have length at least $|\varphi(u,v)|$ since it follows $\varphi$, and it may also take a detour around every attachment corner $a_t\in A(u,v)$.  If it takes this detour at some $a_t\in A(u,v)$, then it does so by walking around $\partial C_t$ which requires an additional $|\partial C_t|$ edges.
...
$2\sum_{a_t\in A(a_1,u)}|\partial C_t| \le 2\sum_{t=1}^r |\partial C_t| < 4n$.
...
\begin{align*}
   &\quad {} \  \sigma_{G}(a_1,v)-\sigma_G(a_1,u) \\
...
Given an embedded path $R = (\rho_1, \rho_2, \ldots, \rho_t)$ that passes through all attachment corners of $G$, we say that $C_i$ \emph{lies to the right of} $R$ if (1) $a_i$ is the only vertex of $C_i$ that belongs to $V(R)$, and (2) if $a_i = \rho_j$ for some $j\in\{1,\ldots,t\}$, then either $j\in \{1,t\}$ or $\rho_{j-1}$ and $\rho_{j+1}$ appear consecutively in this order when sorting---in the graph $G\cup R$---the neighbors of $a_i$ in clockwise order around $a_i$; see Figure~\ref{fig:Neighborhood} (left).
...
We say that two points in the plane are \emph{$R$-visible} if the open straight-line segment joining them does not intersect $R$.
...
\[ O\left(|\varphi(a_j, a_{j+1})| + \sum_{a_i\in A(a_j, a_{j+1})} |\partial C_i|\right) = 
\]
$$O(\sigma_G(a_j, a_{j+1})) \enspace .$$
...
Therefore, after increasing $\varepsilon$  and reducing $\delta$, we preserve the escape invariant for the next iteration of the algorithm.
...
%%% The following is no longer part of the statement of Lemma, so doesn't need
%% to be proved.
...
\begin{figure}[h]
   \centering{\includegraphics[width=.48\textwidth]{img/big-example}}
...
the outer face.  
%(Later, we will show that this is not a real restriction; one can apply 
%the same algorithm to each face of $\mathcal G$.)
...
\subsection{Handling interior components}\label{sec:Handling interior components}
...
\begin{figure*}
  ...
  \caption{\small In the construction in Theorem~\ref{thm:lower-bound}, all drawings use the same set of vertices and line segments and the drawing of a path that joins $\mathcal C_i$ to $\mathcal C_j$ must travel around all paths drawn between the drawing of $\mathcal C_i$ and $\mathcal C_j$.}
...
\begin{proof}
  ...
  $$
  \begin{array}{r}
     \Pr\left\{\max\left\{\left|\pi^{(s)}_i-\pi^{(s)}_j\right|\colon s\in\{1,\ldots,k\} \right\} \le t\right\}\\
 \le  (2t/(r-1))^k = \frac{1}{2(r-1)} \enspace .
   \end{array}
  $$
...
same point set. The point set consists of the vertices of
...
it is a tree; $\mathcal G$ contains no cycles, so any cycle in $\mathcal H$ contains an edge not in $\mathcal G$ that could be removed.  Now, observe that if we traverse the outer face of (any planar drawing of) $\mathcal H$ then we obtain a non-simple path, $P$, that traverses each edge of $\mathcal{H}$ exactly twice. If we consider the set of maximal subpaths of $P$ with no vertex of $\mathcal G$ in their interior, we obtain a set of $r-1$ edge-disjoint paths, $Q_1,\ldots,Q_{r-1}$ and, for every component $\mathcal C_i$ of $\mathcal G$, there is a vertex of $\mathcal C_i$ that is an endpoint of at least one such path.  Therefore, from the preceding discussion, the total length of $Q_1\ldots,Q_{r-1}$ is $\Omega(nr^{1-1/k})$.  But since each edge of $\mathcal H$ appears at most twice in these subpaths,  $\mathcal H$ has $\Omega(nr^{1-1/k})$ edges.  Because $\mathcal H$ is a tree, it has $\Omega(nr^{1-1/k})$ vertices.
\qed\end{proof}
