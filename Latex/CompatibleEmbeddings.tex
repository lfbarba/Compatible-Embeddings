...
$O(n^2)$ vertices are sometimes necessary.  Kranakis and Urrutia
...
Our upper bound is obtained, roughly, by the following argument (assuming
all components of $\mathcal G$ are on the outer face of each embedding,
$G_j$): For each component, $\mathcal C_i$, of $\mathcal G$ we select
a distinguished \emph{corner} $a_i$ of $G_1$ (a space between two
consecutive edges incident on some vertex on the outer face). The corner
$a_i$ is called the \emph{attachment corner} for component $\mathcal C_i$.
Notice that, since $G_1,\ldots,G_k$ are isomorphic, $a_i$ appears as a
corner in each of $G_1,\ldots,G_k$.
...
outer face of the resulting graph to obtain a weakly-simple path, $P_j$,
that visits every corner.  In particular, this path defines an integer
distance $d_j(a_\ell,a_m)$ for any two corners $a_\ell$ and $a_m$.
...
side-length $O(n)$, it has a spanning path of length $O(nr^{1-1/k})$; this
is an old result of So and So \cite{x}.  This implies that $\mathcal G$
can be made connected with a collection of $r-1$ paths, whose endpoints
are the corners $a_1,\ldots,a_r$, having total length $O(nr^{1-1/k})$,
and that each of these paths can be embedded in a planar fashion in each
of $G_1,\ldots,G_k$.

At this point, all that remains is to show that these $r-1$ paths can each be embedded in each of $G_1,\ldots,G_k$ without crossing each other.  This part of the proof involves carefully winding these paths around the components in $G_1,\ldots,G_k$. This part of the proof resembles the first part of the proof of Babikov \etal\ \cite{souvaine.wenger:constructing}.
