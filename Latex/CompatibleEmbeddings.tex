...
it connected and in such a way that these vertices and edges can added to
...
$P$ is $O(n^{2-1/k})$~\cite{steele.snyder:worst}. What's more, a path
of length $O(n^{2-1/k})$ can be obtained from an approximate minimum
spanning tree of $X$; this approximate MST, and the resulting path, $P$,
can be computed in $O(n\log n)$ time \cite{callahan.kosaraju:faster}.
