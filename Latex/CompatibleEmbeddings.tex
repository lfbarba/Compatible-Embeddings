...
Furthermore, this bound is tight; for every $r\in\{2,\ldots,\lfloor n/3\rfloor\}$ and $k\ge 2$, there exists a graph $\mathcal G$
...
Finally, several papers have dealt with the problem of increasing the
connectivity of a (single) geometric planar graph while adding few
vertices and edges.  Abellenas \etal~\cite{aghtu-acgg-08} consider the
problem of adding edges to a planar drawing in order to make it 2-edge
connected \cite{aghtu-acgg-08} and showed that $\lfloor(2n-2)/3\rfloor$
edge are sometimes necessary and $6n/7$ edges are always sufficient.
T\'oth \cite{t-capsg-12} later obtained the tight upper-bound of
$\lfloor(2n-2)/3\rfloor$ for the same problem.  Rutter and Wolff
\cite{rw-acpgg-12} show that the finding the minimum number of edges
required to achieve 2-edge connectivity is NP-hard.
