...
drawings isomorphic to the augmentation of $\mathcal G$.  We show that
adding $O(k^{1/2}nr^{1-1/k})$ edges and vertices to $\mathcal{G}$
is always sufficient and that adding $\Omega(nr^{1-1/k})$ edges and vertices
is sometimes necessary to achieve this goal.  These bounds hold for all $1 < r < \Omega(n)$
and $k>1$.
\end{abstract}
...
showed such a transformation always exists.  Since then, a sequence of results has shown
that such transformations can be done efficiently, so that the
motion can be described concisely \cite{alamdari.angelini.ea:morphing,angelini.dalozzo.ea:morphing,grunbaum.shephard:geometry,thomassen:deformations}.  The most recent such result
...
This motivates the theoretical question studied in the current paper:  How can we add
...
Two planar drawings, $\varphi_1$ and $\varphi_2$,
...
Given $k>1$ geometric planar isomorphic embeddings $G_1, \ldots, G_k$
of $\mathcal G$, a \emph{compatible augmentation}, $\mathcal H$, of
$\mathcal G$ is a supergraph of $\mathcal G$ such that (1) $\mathcal H$
is connected, and (2) there exist geometric planar isomorphic embeddings,
...
compatible augmentation of size $O(k^{1/2}nr^{1-1/k})$ and that this bound
is tight for constant values of $k$; there exists a graph $\mathcal G$
with $r$ components and $k$ drawings for which any compatible augmentation
has size $\Omega(nr^{1-1/k})$.  These bounds hold for all $k>1$ and for $r$ ranging from $2$ to $\Omega(n)$.
...
lives in a hypercube of side-length $O(n)$, a classic result on 
the Euclidean Travelling Salesman Problem \cite{few:shortest} implies that 
it has a spanning path of
Euclidean length $O(k^{1/2}nr^{1-1/k})$.  This implies that $\mathcal G$ can be made
connected with a collection of $r-1$ paths, whose endpoints are the
corners $a_1,\ldots,a_r$, having total size $O(k^{1/2}nr^{1-1/k})$, and
...
$P$ is $O(k^{1/2}n^{2-1/k})$~\cite{few:shortest}.
...
For each $i\in \{1,\dots,n\}$, we can construct a path $R_i$ of length $O(k^{1/2}n^{2-1/k})$ that connects every point of $G_i$ so that $G_i\cup R_i$ is plane. Moreover, for any distinct $i,j\in \{1,\dots,n\}$, $G_i\cup R_i$ and $G_j\cup R_j$ are~isomorphic.
\end{theorem}
\begin{proof}
...
in $\mathcal{H}$ is proportional to the length of $P$, which is $O(k^{1/2}n^{2-1/k})$.
...
has length $O(k^{1/2}n_fr^{1-1/k})$.  We then repeat this step on each face.
The result is a connected augmentation of $\mathcal G$ whose total size
is $O(k^{1/2}Nr^{1-1/k})$ where $N=O(n)$ is the total size of all faces.
...
\begin{enumerate}
  \item Finding connected plane supergraphs $G_1^*,\ldots,G_k^*$
  ...
  $O(k^{1/2}n^{2-1/k})$ can be obtained from an (approximate) minimum spanning
  ...
  Each of these paths is easily constructed in $O(k^{1/2}n^{2-1/k})$ time
...
\begin{theorem}
  ...
  constant, then the augmentation can be computed in $O(k^{1/2}nr^{1-1/k})$
  time.
\end{theorem}
...
\begin{figure}
  ...
  \caption{In the construction in Theorem~\ref{thm:lower-bound}, all drawings use the same set of vertices and line segments and the drawing of a path that joins $\mathcal C_i$ to $\mathcal C_j$ must travel around all paths embedded between the drawing of $\mathcal C_i$ and $\mathcal C_j$.}
...
We have shown that there always exists a compatible augmentation of size
$O(k^{1/2}nr^{1-1/k})$ and that, in some cases, a compatible augmentation
must have size at least $\Omega(nr^{1-1/k})$.  This result is tight for $k\in O(1)$, but leaves a factor of $\sqrt{k}$ between the upper and lower bounds.

\noindent\textbf{Open Problem:} Close the gap between the 
$O(k^{1/2}nr^{1-1/k})$ upper bound and the $\Omega(nr^{1-1/k})$ lower bound given here.

We suspect that the lower bound is correct, and that the factor of $\sqrt{k}$ in our upper-bound can be removed by a more careful analysis of the point sets generated by our lifting into $k$ dimensions.
...
\begin{lemma}\label{lemma:tsp}
  ...
  $p_1,\ldots,p_n$ of $P$ whose length under the uniform (a.k.a.,
  $L_\infty$) norm is at most $cr^{1-1/k}$, where $c$ is a
  universal constant. In particular, $c$ does not depend on $k$ or $r$.
\end{lemma}

\begin{proof}
This proof is a rehashing of an argument used by Moran \cite{moran:on}.
First, we note that it is sufficient to upper-bound the $L_\infty$
length of the minimum spanning-tree of $P$, since this
can be transformed into a path of at most twice its length.

For any point $p\in\R^k$, the \emph{uniform ball} of radius $d$,
defined as
\[
...
step of this algorithm, the set $P$ contains $r-i+1$ points, the total $L_\infty$ length of all the edges added to this tree is
\begin{align*}
   \sum_{i=2}^{r} 2i^{-1/k} 
...
\begin{lemma}\label{lemma:tsp}
  Let $P$ be a set of $r\ge 2$ points contained in 
  cube $[0,n]^k$, for $k\ge 2$.  Then there exists a spanning path
  $p_1,\ldots,p_n$ of $P$ whose length under the uniform (a.k.a.,
  $L_\infty$) norm is at most $cnr^{1-1/k}$, where $c$ is a
  universal constant. In particular, $c$ does not depend on $k$, $r$, or $n$.
\end{lemma}
