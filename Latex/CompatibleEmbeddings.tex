...
We consider the following problem: We are given a (not necessarily connected) planar graph $\mathcal{G}$ and $k$ isomorphic planar drawings $G_1,\ldots,G_k$ of $\mathcal{G}$. We wish to add vertices and edges to $\mathcal{G}$ to make it connected and in such a way that these vertices and edges can added to the drawings $G_1,\ldots,G_k$ while preserving planarity.  We show that adding $O(nr^{1-1/k})$ edges and vertices to $\mathcal{G}$ is always sufficient and sometimes necessary to achieve this goal.
\end{abstract}
...
\cite{angelini.dalozzo.ea:morphing} shows that any planar embedding of
a $n$-vertex connected planar graph can be morphed into any isomorphic
embedding of the same planar using a sequence of $O(n)$ \emph{linear
...
Formally, an \emph{embedding} of a graph $\mathcal{G}=(V,E)$
is a one-to-one function $\varphi\colon V\to\R^2$.  An embedding
...
and $\varphi(y)$.  Two planar embeddings, $\varphi_1$ and $\varphi_2$,
of $\mathcal{G}$ are \emph{isomorphic} if there is a continuous family
of planar embeddings $\{\varphi^{(t)} \colon 0\le t\le 1\}$ of $\mathcal{G}$
such that $\varphi^{(0)}=\varphi_1$ and $\varphi^{(1)}=\varphi_2$.

Throughout this paper we will be slightly less formal to avoid
repeatedly referencing embedding functions like $\varphi$, $\varphi_1$
and $\varphi_2$.  Instead, we will talk about a graph $\mathcal{G}$
and $k$ isomorphic geometric graphs $G_1,\ldots,G_k$.  This should be
understood to mean that each $G_i$ is the embedding of $\mathcal{G}$
...
G$, a \emph{compatible augmentation}, $\mathcal H$, of $\mathcal G$ is a
supergraph of $\mathcal G$ such that (1) $\mathcal H$ is connected, and
(2) there exist geometric planar isomorphic embeddings, $H_1, \ldots,
H_k$, of $\mathcal H$ such that $H_i\supset G_i$ for every $1\leq i\leq
k$.  In this paper, we show that $\mathcal{G}$, if $\mathcal{G}$ has $r$
...
two drawings $P$ and $Q$) so that $G$ becomes a near-triangulation,
...
Next, for each embedding, $G_j$, we add $r-1$ edges to create a spanning
tree of the components of $G_j$ and then traverse the boundary of the
outer face of the resulting graph to obtain a polygonal path, $P_j$,
of length $O(n)$ that comes close to every corner.  The path $P_j$ is
then used to define an integer distance $d_j(a_\ell,a_m)$ for any two
corners $a_\ell$ and $a_m$.

Next, we take a leap into $k$ dimensions by using the distance
functions $d_1,\ldots,d_k$ to produce an $k$-dimensional point set
$P=\{p_1,\ldots,p_r\}$ that lives in a hypercube of side-length $O(n)$.
This mapping has the property that, by adding a path of length $O(\|p_\ell
p_m\|)$ to $\mathcal G$, the corners $a_\ell$ and $a_m$ can be joined
in each of $G_1,\ldots,G_k$ while preserving planarity.

Now, since the point set $P$ has $r$ points and lives in a hypercube of
side-length $O(n)$, a result of Steele and Snyder \cite{steele.snyder:worst} (see also Bern and Eppstein \cite{bern.eppstein:worst}) implies that it has a spanning path of length $O(nr^{1-1/k})$.  This implies that $\mathcal G$ can be made
...
regular $\lfloor n/r\rfloor$-gons.
...
As a warmup, we consider a graph containing $n$ vertices and no edges, i.e., a graph that consists of $n$ (trivial) connected components. 
Before constructing the compatible augmentations, 
we provide a subroutine that constructs a ``short'' plane spanning path of a point set. 
...
Given a point $v\in S$, let $rank(v)$ denote the number of points of $S$ that lie to the left of (have smaller $x$-coordinate than) $v$.
